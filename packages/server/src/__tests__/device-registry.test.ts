...
describe('DeviceRegistry', () => {
  ...
  describe('registerDevice', () => {
    ...
    it('daemon 注册时携带 accessToken 应自动注册 Token', () => {
      const ws = createMockWs();
      registry.registerDevice(ws, 'daemon-1', 'daemon', 'pk-1', 'mytermux-test-token');
  ...
  describe('unregisterDevice', () => {
    ...
    it('daemon 重连时应清除 Token 待清理标记', () => {
      const ws1 = createMockWs();
      registry.registerDevice(ws1, 'daemon-1', 'daemon', 'pk-1', 'mytermux-test-token');
      registry.unregisterDevice('daemon-1');

      // 重连
      const ws2 = createMockWs();
      registry.registerDevice(ws2, 'daemon-1', 'daemon', 'pk-1', 'mytermux-test-token');
  ...
  describe('validateAccessToken', () => {
    it('Token 有效时应建立认证关系并返回 daemonId', () => {
      ...
      registry.registerDevice(daemonWs, 'daemon-1', 'daemon', 'pk-d1', 'mytermux-valid-token');
      registry.registerDevice(clientWs, 'client-1', 'client', 'pk-c1');

      const result = registry.validateAccessToken('mytermux-valid-token', 'client-1');
    ...
    it('client 未注册时应返回 null', () => {
      const daemonWs = createMockWs();
      registry.registerDevice(daemonWs, 'daemon-1', 'daemon', 'pk-d1', 'mytermux-token');

      const result = registry.validateAccessToken('mytermux-token', 'unregistered-client');
      expect(result).toBeNull();
    });
    ...
    it('daemon 离线时应返回 null', () => {
      ...
      const result = registry.validateAccessToken('mytermux-token', 'client-1');
      expect(result).toBeNull();
    });
    ...
    it('deviceType 非 client 的设备不应通过认证', () => {
      ...
      const result = registry.validateAccessToken('mytermux-token', 'daemon-2');
      expect(result).toBeNull();
    });
    ...
    it('同一 Token 可被多个 client 使用', () => {
      ...
      registry.registerDevice(daemonWs, 'daemon-1', 'daemon', 'pk-d1', 'mytermux-shared-token');
      registry.registerDevice(client1Ws, 'client-1', 'client', 'pk-c1');
      registry.registerDevice(client2Ws, 'client-2', 'client', 'pk-c2');

      expect(registry.validateAccessToken('mytermux-shared-token', 'client-1')).toBe('daemon-1');
      expect(registry.validateAccessToken('mytermux-shared-token', 'client-2')).toBe('daemon-1');
