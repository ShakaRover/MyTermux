...
- `relay`：设备中继、daemon profile 管理、ws-ticket 签发、Web 认证 API
- `daemon`：终端会话执行与 `MYTERMUX_DAEMON_TOKEN` 管理

## 职责与数据库边界

- Web（浏览器）：本地数据库（IndexedDB，库名 `mytermux_web_db`）
  - 仅存储快捷键、Relay 地址与 Web Link Token 等本地偏好
...
- Relay 管理 API：提供 `/api/web-auth/*`、daemon/profile/ws-ticket
...
- relay: `relay.db`, `web.db`, `relay.pid`, `relay.log`
