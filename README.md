...
- `web`：界面管理（登录通过服务端 `web.db`）
- `server`：设备中继、daemon profile 管理、ws-ticket 签发、Web 认证 API（实现目录仍为 `packages/relay`）
- `daemon`：终端会话执行与 `MYTERMUX_DAEMON_TOKEN` 管理

## 职责与数据库边界

- Web（浏览器）：本地数据库（IndexedDB，库名 `mytermux_web_db`）
  - 仅存储快捷键、Server 地址与 Web Link Token 等本地偏好
...
- `MYTERMUX_WEB_LINK_TOKEN`：Web 访问 Server 管理 API 与 ws-ticket 的链路授权 token（Server 配置）
...
- Server API 鉴权：当配置 `MYTERMUX_WEB_LINK_TOKEN` 时，管理 API 需 `x-mytermux-web-link-token` 请求头
- WebSocket 准入：`/api/ws-ticket` 一次性 ticket（60 秒）

## Monorepo

- `packages/shared`: 协议、类型、加密
- `packages/relay`: Server HTTP/API + WebSocket
...
# 兼容旧脚本：bash ./scripts/daemon/set-relay-token.sh --clear
```
...
2. 首次登录必须先修改账号密码
3. 在 `/daemons` 配置 Server 地址与 `MYTERMUX_WEB_LINK_TOKEN`
...
# 兼容旧别名：pnpm daemon:relay-token:set -- <token>
```
