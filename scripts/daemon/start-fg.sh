...
SERVER_URL="${SERVER_URL:-${RELAY_URL:-ws://${SERVER_HOST}:${SERVER_PORT}}}"
DAEMON_HOST="${DAEMON_HOST:-127.0.0.1}"
DAEMON_PORT="${DAEMON_PORT:-62300}"

daemon_args=(start -f --server "$SERVER_URL" --listen-host "$DAEMON_HOST" --listen-port "$DAEMON_PORT")
if [[ -n "${MYTERMUX_DAEMON_LINK_TOKEN:-}" ]]; then
  daemon_args+=(--daemon-link-token "$MYTERMUX_DAEMON_LINK_TOKEN")
fi

echo "[daemon] 前台启动: server=${SERVER_URL}, listen=http://${DAEMON_HOST}:${DAEMON_PORT}"
